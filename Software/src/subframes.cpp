...
uint8_t FLACSubFrameHeader::getFixedOrder() { return _subFrameType & 0x7; }
    ...
    switch (_predictorOrder) {
        ...
        case 3:
            for (i = 3; i < _blockSize; i++)
                data[i] = residuals[i - 3] + 3 * data[i - 1] - 3 * data[i - 2] +
                          data[i - 3];
            break;
        case 4:
            for (i = 4; i < _blockSize; i++)
                data[i] = residuals[i - 4] + 4 * data[i - 1] - 6 * data[i - 2] +
                          4 * data[i - 3] - data[i - 4];
            break;
        default:
            break;
...
void FLACSubFrameLPC::setLPCOrder(uint8_t lpcOrder) { _lpcOrder = lpcOrder; }
...
int FLACSubFrameLPC::read(BitReader &fr) {
    ...
    for (i = 0; i < _lpcOrder; i++) {
        fr.read_bits_signed(
            data + i,
            _bitsPerSample);  // Store these samples ... (are they signed???)
    }
...
int FLACSubFrameLPC::read(BitReader &fr, int32_t *data) {
    ...
    for (i = _lpcOrder; i < _blockSize; i++) {
        sum = 0;
        for (j = 0; j < _lpcOrder; j++) sum += _qlpCoeff[j] * data[i - j - 1];
...
FLACSubFrameConstant::FLACSubFrameConstant(uint8_t bitsPerSample,
                                           uint32_t blockSize) {
    _bitsPerSample = bitsPerSample;
    _blockSize = blockSize;
...
void FLACSubFrameConstant::reconstruct(uint8_t bitsPerSample,
                                       uint32_t blockSize) {
    _bitsPerSample = bitsPerSample;
    _blockSize = blockSize;
...
FLACSubFrameVerbatim::FLACSubFrameVerbatim(uint8_t bitsPerSample,
                                           uint32_t blockSize) {
    _bitsPerSample = bitsPerSample;
    _blockSize = blockSize;
}
...
void FLACSubFrameVerbatim::reconstruct(uint8_t bitsPerSample,
                                       uint32_t blockSize) {
    _bitsPerSample = bitsPerSample;
    _blockSize = blockSize;
}
