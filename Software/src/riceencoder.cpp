...
std::vector<uint8_t> RiceEncoder::calc_best_rice_params(int32_t data[],
                                                        int samples,
                                                        uint32_t &total_bits) {
    // Test all 8 rice params
    auto prefixsums =
        std::vector<std::vector<int>>(8, std::vector<int>(samples));
    ...
    for (int i = 1; i < samples; i++)
        for (unsigned r = 0; r < prefixsums.size(); r++)
            prefixsums[r][i] =
                prefixsums[r][i - 1] + calc_rice_bits(data[i], r);
    ...
    auto rice_params = std::vector<std::vector<int>>(
        npartitions, std::vector<int>(1 << npartitions - 1));
    ...
    for (int p = 0; p < npartitions; p++) {  // For each partition size
        ...
        int spp = samples / (1 << p);
        part_size_sums[p] =
            4 * (1 << p);  // This is the overhead of parameter spec

        for (int i = 0; i < (1 << p); i++) {
            for (int r = 0; r < 8; r++)
                bit_sums[r] =
                    prefixsums[r][(i + 1) * spp - 1] - prefixsums[r][i * spp];

            auto min_bits = std::distance(
                bit_sums.begin(),
                std::min_element(bit_sums.begin(), bit_sums.end()));
            rice_params[p][i] = min_bits;
            part_size_sums[p] += bit_sums[min_bits];
        }
    }

    auto min_part_size = std::distance(
        part_size_sums.begin(),
        std::min_element(part_size_sums.begin(), part_size_sums.end()));
