...
int FixedEncoder::calc_best_order(int32_t data[], int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        ...
        if (i > 0)
            sum_errors[1] += abs(data[i] - data[i - 1]);
        if (i > 1)
            sum_errors[2] += abs(data[i] - 2 * data[i - 1] + data[i - 2]);
        if (i > 2)
            sum_errors[3] += abs(data[i] - 3 * data[i - 1] + 3 * data[i - 2] - data[i - 3]);
        if (i > 3)
            sum_errors[4] += abs(data[i] - 4 * data[i - 1] + 6 * data[i - 2] - 4 * data[i - 3] + data[i - 4]);
    }
