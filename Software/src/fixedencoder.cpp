...
int FixedEncoder::calc_best_order(int32_t data[], int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        ...
        if (i > 1)
            sum_errors[2] += abs(data[i] - 2 * data[i - 1] + data[i - 2]);
        if (i > 2)
            sum_errors[3] +=
                abs(data[i] - 3 * data[i - 1] + 3 * data[i - 2] - data[i - 3]);
        if (i > 3)
            sum_errors[4] += abs(data[i] - 4 * data[i - 1] + 6 * data[i - 2] -
                                 4 * data[i - 3] + data[i - 4]);
    }

    return std::distance(
        sum_errors.begin(),
        std::min_element(sum_errors.begin(), sum_errors.end()));
}

/* Assume residuals is same size as data */
void FixedEncoder::calc_residuals(int32_t *data, int32_t *residuals,
    ...
    for (int i = order; i < samples; i++) {
        switch (order) {
            ...
            case 3:
                ...
                break;
            case 4:
                residuals[i] = data[i] - 4 * data[i - 1] + 6 * data[i - 2] -
                               4 * data[i - 3] + data[i - 4];
                break;
        }
    }
