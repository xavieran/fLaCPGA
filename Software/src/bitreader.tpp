...
template <typename T>
int BitReader::read_word_LE(T *x) {
...
template <typename T>
int BitReader::read_words_LE(T *x, uint64_t words) {
...
template <typename T>
int BitReader::read_word_LE_aligned(T *x) {
    assert(this->is_byte_aligned());  // Only execute this when byte aligned...
    T result = 0;
    unsigned bytes = sizeof(T);
    for (unsigned i = 0; i < bytes; i++) {
...
template <typename T>
int BitReader::read_words_LE_aligned(T *x, uint64_t words) {
    for (unsigned i = 0; i < words; i++) {
...
template <typename T>
int BitReader::read_chunk(T *dst, int nmemb) {
    if (this->bytes_left() == 0) this->refill_buffer();
...
template <typename T>
int BitReader::read_bits(T *x, uint8_t nbits) {
    ...
    while (nbits > 0) {
        ...
        if (nbits > bits_left_in_byte) {
            t <<= bits_left_in_byte;
            t |= ((((T)(*_curr_byte)) & BYTE_MASK[bits_left_in_byte]) >>
                  (8 - bits_left_in_byte));
            ...
            nbits = 0;
            if (_bitp % 8 == 0)
                _curr_byte++;  // Always leave the buffer on the next byte...
        }
    }
    *x = t;
    return true;
...
template <typename T>
int BitReader::read_bits_signed(T *x, uint8_t nbits) {
    T ux, mask;
    read_bits(&ux, nbits);
...
template <typename T>
int BitReader::read_bits_unary(T *x) {
    int c = 0;
    uint8_t b = 0;

    while (1) {
        if (!read_bits(&b, 1)) return false;
