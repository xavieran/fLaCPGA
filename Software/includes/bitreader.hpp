...
// 1MB default buffer size, should conduct experiments to see effect on
// performance
...
class BitReader {
   public:
    ...
    template <typename T>
    int read_bits(T *x, uint8_t bits);
    template <typename T>
    int read_bits_signed(T *x, uint8_t nbits);
    template <typename T>
    int read_bits_unary(T *x);
    template <typename T>
    int read_word_LE(T *x);
    template <typename T>
    int read_words_LE(T *x, uint64_t words);

    template <typename T>
    int read_word_LE_aligned(T *x);
    template <typename T>
    int read_words_LE_aligned(T *x, uint64_t words);

    template <typename T>
    int read_chunk(T *dst, int nmemb);
