...
class FLACMetaStreamInfo : public FLACMetaDataBlock {
  ...
  public:
    FLACMetaStreamInfo();
    FLACMetaStreamInfo(uint16_t min_blk_sz, uint16_t max_blk_sz, uint32_t min_frm_sz, uint32_t max_frm_sz,
                       uint32_t samp_rate, uint8_t num_chan, uint8_t bps, uint64_t total_samp, uint64_t MD5u,
                       uint64_t MD5l);
