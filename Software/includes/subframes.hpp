...
class FLACSubFrame {
   public:
    FLACSubFrameHeader *getHeader() { return _header; }

    void setHeader(FLACSubFrameHeader *h) { _header = h; }
...
class FLACSubFrameFixed {
   ...
   public:
    ...
    FLACSubFrameFixed(uint8_t bitsPerSample, uint32_t blockSize,
...
class FLACSubFrameLPC {
   ...
   public:
    FLACSubFrameLPC();
    FLACSubFrameLPC(uint8_t bitsPerSample, uint32_t blockSize,
                    uint8_t lpcOrder);
    void reconstruct(uint8_t bitsPerSample, uint32_t blockSize,
                     uint8_t lpcOrder);
