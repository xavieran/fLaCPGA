...
class FLACFrameHeader {
  public:
    FLACFrameHeader();
    FLACFrameHeader(uint8_t blk_strat, uint32_t samp_rate, uint8_t chan_assign, uint8_t samp_size, uint64_t number,
                    uint16_t blk_size);
    void reconstruct();
    void reconstruct(uint8_t blk_strat, uint32_t samp_rate, uint8_t chan_assign, uint8_t samp_size, uint64_t number,
                     uint16_t blk_size);
    int isLast();
  ...
  private:
     ...
     * 15 bits to be distinguishable from the start of an MPEG audio frame (see
     * also). */
    uint8_t _reserved1;
     ...
     *      8 channels: front left, front right, front center, LFE, back left,
     * back right, side left, side right
     *  1000 : left/side stereo: channel 0 is the left channel, channel 1 is the
     * side(difference) channel
     *  1001 : right/side stereo: channel 0 is the side(difference) channel,
     * channel 1 is the right channel
     *  1010 : mid/side stereo: channel 0 is the mid(average) channel, channel 1
